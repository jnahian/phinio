 ...
 * force-close because `resumePausedMutations()` couldn't find a function to
 * call. Values mirror apps/web's registry so both clients share one
 * vocabulary in logs and debugging.
 */
 ...
 * `networkMode: 'offlineFirst'` so taps while offline pause-and-queue
 * rather than rejecting immediately. This MUST run before the MMKV
 * persister finishes restoring; otherwise rehydrated mutations have no
 * `mutationFn` to call and `resumePausedMutations()` silently does
 * nothing. Wired at module scope in `app/_layout.tsx`.
 */
