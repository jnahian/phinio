...
export const dashboardRouter = router({
  stats: protectedProcedure.query(async ({ ctx }): Promise<DashboardStats> => {
    const now = new Date()
    ...
    const [
      ...
      ctx.prisma.emiPayment.findMany({
        where: {
      ...
      ctx.prisma.investmentDeposit.findMany({
        where: {
          profileId: ctx.profileId,
          status: { not: 'paid' },
    ...
    const gainLossPercent =
      invested > 0
        ? Math.round(((current + withdrawn - invested) / invested) * 10000) /
          100
        : 0
    ...
    const upcomingEmiItems: UpcomingPaymentItem[] = upcomingEmiRows.map(
      (p) => ({
        id: p.id,
        kind: 'emi',
        emiId: p.emiId,
