...
- Add deps: `@tanstack/query-persist-client-core`, `idb-keyval`, `superjson`.
- In `src/integrations/tanstack-query/root-provider.tsx`:
  ...
  - Build an IndexedDB persister using `idb-keyval` keyed at `phinio-query-cache`.
  - Wrap the client with `persistQueryClient({ queryClient, persister, maxAge: 7 * 24 * 60 * 60_000, buster: CACHE_SCHEMA_VERSION })` where `CACHE_SCHEMA_VERSION` is a manually-bumped constant (currently `'v1'`). **Do not** use `__APP_VERSION__` as the buster — patch releases must not invalidate users' cached financial data.
  - Use `superjson` for `serialize`/`deserialize` so `Date` (and `BigInt`, `Map`, etc.) survive the round-trip. Plain `JSON.stringify` would silently degrade `Date` to ISO strings and break callsites that expect `Date`.
  - **Wrap the persister setup AND each storage callback in try/catch** — synchronous setup throws (browsers blocking IndexedDB) and runtime async failures (quota, Safari Private Browsing) both need to fall back cleanly. A failure logs once via `console.warn`; the app continues with in-memory cache.
...
| 2   | **Decimal arithmetic:** strings on the wire; server-side aggregations and comparisons use `Prisma.Decimal`; client-side optimistic totals come from `emi-calculator`'s pre-computed `totalPayment` string. Never coerce a `Decimal(15,2)` field to JS `Number` for arithmetic — per the rule in CLAUDE.md. | `Prisma.Decimal` ships with `@prisma/client` so no extra dep. Closed-form formulas inside `emi-calculator.ts` round at every boundary and return strings (acceptable, well below `Number.MAX_SAFE_INTEGER`). Aggregations like "sum of withdrawals" or "remaining balance" go through `Prisma.Decimal`.                                                                |
